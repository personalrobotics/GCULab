...
class Se3KeyboardCfg(DeviceCfg):
    ...
    hold_delay: float = 0.1  # Time in seconds before key is considered "held" for continuous movement
...
class Se3Keyboard(DeviceBase):
    ...
    * gripper: a binary command to open or close the gripper.
    ...
    Key behavior:
    - Single press: Immediately registers a one-time movement command
    - Hold: After a configurable delay (default 0.2 seconds), begins continuous movement
    This allows both precise single-step movements and smooth continuous control.
    ...
    def __init__(self, cfg: Se3KeyboardCfg):
        ...
        self.hold_delay = cfg.hold_delay
    ...
    def reset(self):
        ...
        self._delta_pos = np.zeros(3)  # (x, y, z)
        self._delta_rot = np.zeros(3)  # (roll, pitch, yaw)
        # reset key hold tracking
        self._key_press_times.clear()
        self._keys_held.clear()
        # reset single press tracking
    ...
    def advance(self) -> torch.Tensor:
        ...
        command = np.concatenate([total_pos, rot_vec])
            ...
            if key_name in self._key_press_times:
                ...
                if time_held >= self.hold_delay:
                    ...
                    if key_name in ["W", "S", "A", "D", "Q", "E"]:
                        if key_name not in self._active_pos_keys:
                            self._delta_pos += self._INPUT_KEY_MAPPING[key_name]
                            self._active_pos_keys.add(key_name)
                    elif key_name in ["Z", "X", "T", "G", "C", "V"]:
                        if key_name not in self._active_rot_keys:
                            self._delta_rot += self._INPUT_KEY_MAPPING[key_name]
                            self._active_rot_keys.add(key_name)
    ...
    def _on_keyboard_event(self, event, *args, **kwargs):
        ...
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            ...
            elif event.input.name in ["W", "S", "A", "D", "Q", "E", "Z", "X", "T", "G", "C", "V"]:
                # CRITICAL: Always register single press command immediately
                # This ensures rapid presses are never missed, regardless of hold state
        ...
        # remove the command when un-pressed
            ...
            if event.input.name in self._keys_held:
            ...
            if event.input.name in ["W", "S", "A", "D", "Q", "E"]:
                if event.input.name in self._active_pos_keys:
                    ...
                    self._active_pos_keys.remove(event.input.name)
            elif event.input.name in ["Z", "X", "T", "G", "C", "V"]:
