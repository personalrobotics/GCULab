...
def spawn_multi_usd_from_dist_file(
    prim_path: str,
    cfg: wrappers_cfg.MultiUsdFromDistFileCfg,
    ...
    if cfg.distribution is not None:
        if cfg.distribution.dim() != 1 or cfg.distribution.numel() != len(usd_paths):
            raise ValueError("Distribution must be a 1D tensor matching the number of USD paths.")

    # make a template usd config
    usd_template_cfg = UsdFileCfg()
    for attr_name, attr_value in cfg.__dict__.items():
        # skip names we know are not present
        if attr_name in ["func", "usd_path", "random_choice", "distribution"]:
            continue
    ...
    # Override selection method to use distribution if provided
    ...
    try:
        ...
        # Restore the original random.choice if we replaced it
        if cfg.distribution is not None and cfg.random_choice:
